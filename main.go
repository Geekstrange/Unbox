...
func addGradient(text string, startRGB, endRGB [3]int) string {
        ...
        for i, r := range text {
                ...
                rVal := int(float64(startRGB[0]) + ratio*(float64(endRGB[0])-float64(startRGB[0])))
                gVal := int(float64(startRGB[1]) + ratio*(float64(endRGB[1])-float64(startRGB[1])))
                bVal := int(float64(startRGB[2]) + ratio*(float64(endRGB[2])-float64(startRGB[2])))
                result.WriteString(fmt.Sprintf("\033[38;2;%d;%d;%dm%c", rVal, gVal, bVal, r))
        }
...
func main() {
        ...
        switch {
        case *version:
                startRGB := [3]int{210, 58, 68}
                endRGB := [3]int{221, 155, 85}
                url := "http://www.apache.org/licenses/LICENSE-2.0"
                // 找到版本信息中URL的位置
                urlIndex := strings.Index(versionText, url)
                if urlIndex == -1 {
                        log.Fatal("未找到URL在版本信息中的位置")
                }
                // 给版本信息添加渐变效果
                gradientVersionText := addGradient(versionText[:urlIndex], startRGB, endRGB)
                // 给URL添加超链接和渐变效果
                gradientUrl := createHyperlink(addGradient(url, startRGB, endRGB), url)
                // 给剩余的版本信息添加渐变效果
                remainingText := addGradient(versionText[urlIndex+len(url):], startRGB, endRGB)
                fmt.Println(gradientVersionText + gradientUrl + remainingText)
        case *help:
                printHelp()
        case *supported:
